...
describe('MetricsComponent', () => {
  ...
  it('should display a table of interfaces and the times completed', async () => {
    ...
    expect(rows[0].cells[0].textContent).toBe(MetricInterfaceName.BASELINE);
    expect(rows[0].cells[1].textContent).toBe('1');
    expect(rows[1].cells[0].textContent).toBe(MetricInterfaceName.RSVP_BASIC);
    expect(rows[1].cells[1].textContent).toBe('2');
    expect(rows[2].cells[0].textContent).toBe(MetricInterfaceName.RSVP_SUBWAY);
